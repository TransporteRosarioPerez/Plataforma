...
function buildStorageKey(clientId: string, fileName: string) {
  ...
  return `proformas/${clientId}/${Date.now()}-${safe}`
}

export async function generateProformaUploadUrl(
  clientId: string,
  fileName: string
  ...
  try {
    ...
    await logAudit({
      action: AUDIT_ACTIONS.proformaUpload,
      entityType: 'proforma',
      summary: 'Inició subida de PDF de proforma',
...
export async function generateProformaDownloadUrl(
  proformaId: string
  ...
  if (error || !data?.file_url) {
    return { error: 'Esta proforma no tiene PDF asociado' }
  }
  ...
  try {
    ...
    await logAudit({
      action: AUDIT_ACTIONS.proformaDownload,
      entityType: 'proforma',
      entityId: proformaId,
      summary: 'Descargó PDF de proforma',
    })
    return {
      success: 'URL generada',
      url,
      fileName: data.file_name ?? 'proforma.pdf',
    }
  } catch (e) {
