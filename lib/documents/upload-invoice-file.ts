import { generateInvoiceUploadUrl } from '@/lib/actions/invoice-file'

export async function appendInvoiceFileToFormData(
  formData: FormData,
  clientId: string,
  file: File | null
  ...
  const upload = await generateInvoiceUploadUrl(clientId, file.name)
